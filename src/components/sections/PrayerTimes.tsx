...
export default function PrayerTimes() {
  ...
  return (
    <section className="py-24 px-6 bg-bg-secondary border-y border-border relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row items-end justify-between border-b border-border pb-6 mb-12 gap-8">
          <div>
            ...
            <h2 className="text-4xl font-display font-bold text-text-primary">Jadwal Waktu Shalat</h2>
          </div>
        ...
        {loading ? (
          <div className="h-32 flex items-center justify-center">
            <div className="w-8 h-8 border-4 border-emerald-primary border-t-emerald-primary rounded-full animate-spin" />
          </div>
        ) : (
            ...
            {prayerList.map((prayer, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    "min-w-[160px] md:min-w-0 snap-center flex flex-col items-center justify-center p-6 rounded-2xl transition-all duration-300 relative overflow-hidden group border",
                    isActive 
                      ? "bg-emerald-primary/10 border-emerald-primary/40 scale-105 z-10" 
                      : "bg-bg-primary border-border"
                  )}
                >
                  <span className={cn(
                    "font-medium mb-2 relative z-10",
                    isActive ? "text-emerald-primary" : "text-text-secondary group-hover:text-text-primary transition-colors"
                  )}>
                    {prayer.name}
                  </span>
                  <span className={cn(
                    "text-3xl font-bold font-display tabular-nums relative z-10",
                    isActive ? "text-text-primary" : "text-emerald-primary/70"
                  )}>
                    {prayer.time}
                  </span>
                </motion.div>
